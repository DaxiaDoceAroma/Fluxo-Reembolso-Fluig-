...
function getModalZoom() {
  ...
  return (config = {}) => {
    ...
    if (!searchField) throw new Error('SearchField é obrigatório');
    ...
    return new Promise((resolve, reject) => {
      ...
      zoomModal = FLUIGC.modal(modalConfig, error => {
        ...
        async function doSearch() {
          ...
          try {
              ...
              filters.forEach(filter =>
                constraints.push(
                  DatasetFactory.createConstraint(
                    ...
                    filter.initialValue,
                    filter.finalValue,
                    filter.type || ConstraintType.MUS,
            ...
            if ($modalSearch.value)
              constraints.push(DatasetFactory.createConstraint(searchField, $modalSearch.value, $modalSearch.value, ConstraintType.MUST, true));
            ...
            resultProxy.data = dsResult.values;
            doFilter(true);
          } catch (error) {
            FLUIGC.toast({
        ...
        function doFilter(initial = false) {
          ...
          resultProxy.filtered = resultProxy.data.filter(item => (item[searchField] || '').toUpperCase().includes(searchValue));
        }
